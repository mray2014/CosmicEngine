...
void WaterHubWorld::Init()
{
	...
	int* levelActive = gameManager->GetActiveLevels(Worlds::Water);
...
void WaterHubWorld::Update(float deltaTime, float totalTime)
{
	...
	for (unsigned int i = 0; i < levels.size(); i++) // SUPER TIME CONSUMING < ---- cost tons of frames
	{
		...
		{
			if ((gameManager->GetActiveLevels(Worlds::Water)[i] >= 1))
			{
				if (i == levelCount - 1)
				{
...
void WaterHubWorld::LoadBackgroundTilePool(std::string textureName)
{
...
void WaterHubWorld::CompletedLevel(int index)
{
