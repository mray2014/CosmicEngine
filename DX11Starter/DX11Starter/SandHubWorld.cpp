...
void SandHubWorld::Init()
{
	...
	int* levelActive = gameManager->GetActiveLevels(Worlds::Sand);
...
void SandHubWorld::Update(float deltaTime, float totalTime)
{
	...
	for (unsigned int i = 0; i < levels.size(); i++) // SUPER TIME CONSUMING < ---- cost tons of frames
	{
		...
		{
			if ((gameManager->GetActiveLevels(Worlds::Sand)[i] >= 1))
			{
				if (i == levelCount - 1)
				{
...
void SandHubWorld::LoadBackgroundTilePool(std::string textureName)
{
...
void SandHubWorld::CompletedLevel(int index)
{
