...
RainbowHubWorld::RainbowHubWorld()
{
	cam = new FreeCamera();
}
...
void RainbowHubWorld::Init()
{
	...
	int* levelActive = gameManager->GetActiveLevels(Worlds::Rainbow);
...
void RainbowHubWorld::Update(float deltaTime, float totalTime)
{
	...
	for (unsigned int i = 0; i < levels.size(); i++) // SUPER TIME CONSUMING < ---- cost tons of frames
	{
		...
		{
			if ((gameManager->GetActiveLevels(Worlds::Rainbow)[i] >= 1))
			{
				if (i == levelCount - 1)
				{
...
void RainbowHubWorld::LoadBackgroundTilePool(std::string textureName)
{
...
void RainbowHubWorld::CompletedLevel(int index)
{
