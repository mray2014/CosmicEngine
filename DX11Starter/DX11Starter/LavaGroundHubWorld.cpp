...
LavaGroundHubWorld::LavaGroundHubWorld()
{
	cam = new FreeCamera();
}
...
void LavaGroundHubWorld::Init()
{
	...
	int* levelActive = gameManager->GetActiveLevels(Worlds::LavaGround);
...
void LavaGroundHubWorld::Update(float deltaTime, float totalTime)
{
	...
	for (unsigned int i = 0; i < levels.size(); i++) // SUPER TIME CONSUMING < ---- cost tons of frames
	{
		...
		{
			if ((gameManager->GetActiveLevels(Worlds::LavaGround)[i] >= 1))
			{
				if (i == levelCount - 1)
				{
...
void LavaGroundHubWorld::LoadBackgroundTilePool(std::string textureName)
{
...
void LavaGroundHubWorld::CompletedLevel(int index)
{
