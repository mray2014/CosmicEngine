...
void LavaHubWorld::Init()
{
	...
	int* levelActive = gameManager->GetActiveLevels(Worlds::Lava);
...
void LavaHubWorld::Update(float deltaTime, float totalTime)
{
	...
	for (unsigned int i = 0; i < levels.size(); i++) // SUPER TIME CONSUMING < ---- cost tons of frames
	{
		...
		{
			if ((gameManager->GetActiveLevels(Worlds::Lava)[i] >= 1))
			{
				if (i == levelCount - 1)
				{
...
void LavaHubWorld::LoadBackgroundTilePool(std::string textureName)
{
...
void LavaHubWorld::CompletedLevel(int index)
{
