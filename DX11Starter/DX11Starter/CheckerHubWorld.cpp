...
CheckerHubWorld::CheckerHubWorld()
{
	cam = new FreeCamera();
}
...
void CheckerHubWorld::Init()
{
	...
	int* levelActive = gameManager->GetActiveLevels(Worlds::Checker);
...
void CheckerHubWorld::Update(float deltaTime, float totalTime)
{
	...
	for (unsigned int i = 0; i < levels.size(); i++) // SUPER TIME CONSUMING < ---- cost tons of frames
	{
		...
		{
			if ((gameManager->GetActiveLevels(Worlds::Checker)[i] >= 1))
			{
				if (i == levelCount - 1)
				{
...
void CheckerHubWorld::LoadBackgroundTilePool(std::string textureName)
{
...
void CheckerHubWorld::CompletedLevel(int index)
{
