...
void GrassHubWorld::Init()
{
	...
	int* levelActive = gameManager->GetActiveLevels(Worlds::Grass);
...
void GrassHubWorld::Update(float deltaTime, float totalTime)
{
	...
	for (unsigned int i = 0; i < levels.size(); i++) // SUPER TIME CONSUMING < ---- cost tons of frames
	{
		...
		{
			if ((gameManager->GetActiveLevels(Worlds::Grass)[i] >= 1))
			{
				if (i == levelCount - 1)
				{
...
void GrassHubWorld::LoadBackgroundTilePool(std::string textureName)
{
...
void GrassHubWorld::CompletedLevel(int index)
{
