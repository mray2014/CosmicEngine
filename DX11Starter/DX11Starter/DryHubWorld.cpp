...
void DryHubWorld::Init()
{
	...
	int* levelActive = gameManager->GetActiveLevels(Worlds::Dry);
...
void DryHubWorld::Update(float deltaTime, float totalTime)
{
	...
	for (unsigned int i = 0; i < levels.size(); i++) // SUPER TIME CONSUMING < ---- cost tons of frames
	{
		...
		{
			if ((gameManager->GetActiveLevels(Worlds::Dry)[i] >= 1))
			{
				if (i == levelCount - 1)
				{
...
void DryHubWorld::LoadBackgroundTilePool(std::string textureName)
{
...
void DryHubWorld::CompletedLevel(int index)
{
