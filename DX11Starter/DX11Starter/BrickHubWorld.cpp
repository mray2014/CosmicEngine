...
void BrickHubWorld::Init()
{
	...
	int* levelActive = gameManager->GetActiveLevels(Worlds::Brick);
...
void BrickHubWorld::Update(float deltaTime, float totalTime)
{
		...
		for (unsigned int i = 0; i < levels.size(); i++) // SUPER TIME CONSUMING < ---- cost tons of frames
		{
			...
			{
				if((gameManager->GetActiveLevels(Worlds::Brick)[i] >= 1))
				{
					if (i == levelCount - 1) 
					{
...
void BrickHubWorld::LoadBackgroundTilePool(std::string textureName)
{
...
void BrickHubWorld::CompletedLevel(int index)
{
